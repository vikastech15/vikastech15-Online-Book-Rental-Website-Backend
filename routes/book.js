...
router.get("/books/:id", async (req, res) => {
  try {
    const { id } = req.params;
    const book = await Listing.findById(id);
...
// CREATE BOOK (Cloudinary Upload)
router.post("/books", upload.array("images", 3), async (req, res) => {
  try {
    const newListing = new Listing(req.body);

    // sale / rent logic
    const forSaleData = req.body?.forSaleData || "rent"; 
    if (req.body.forSaleData != "rent") {
      newListing.forRent = false;
      newListing.forSale = true;
    }
    ...
    // 1. If user provides external thumbnail URL
    if (req.body.thumbnailUrl) {
      imageArray.push({
        url: req.body.thumbnailUrl,
    ...
    // 2. Uploaded images from Cloudinary
    if (req.files && req.files.length > 0) {
      req.files.forEach((file) => {
        imageArray.push({
